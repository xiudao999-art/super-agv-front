(function () {
  ...
  const procedureStatusStorageKey = 'agv-emergency-procedure-status';
  let storedProcedureStatus = {};
  try { storedProcedureStatus=JSON.parse(localStorage.getItem(procedureStatusStorageKey)||'{}'); } catch (error) { storedProcedureStatus={}; }
  const procedureEnabled = new Map(procedures.map(item=>[item.id,storedProcedureStatus[item.id]!==false]));

  function persistProcedureStatus() {
    try { localStorage.setItem(procedureStatusStorageKey,JSON.stringify(Object.fromEntries(procedureEnabled))); } catch (error) { /* 本地存储不可用时仍保留本次页面状态 */ }
  }
  ...
  function renderProcedures(list=procedures) {
    ...
    body.innerHTML=list.map(item=>{const related=tickets[item.id]||[],enabled=procedureEnabled.get(item.id);return `<tr class="clickable-row" data-procedure="${item.id}"><td class="ops-code">${item.id}</td><td><strong class="table-primary">${item.title}</strong><span class="table-secondary">${item.signal}</span></td><td>${scopeChip(item.scope)}</td><td>${item.duty}</td><td>${related.length?related.map(id=>`<a class="ticket-chip procedure-ticket-link" href="exception-recovery.html?ticket=${id}" onclick="event.stopPropagation()">${id}</a>`).join(''):'<span class="table-secondary">—</span>'}</td><td class="procedure-status-cell"><button class="procedure-switch ${enabled?'is-on':''}" type="button" role="switch" aria-checked="${enabled}" aria-label="${enabled?'停用':'启用'}规则 ${item.id}" data-procedure-toggle="${item.id}"><span class="procedure-switch-track" aria-hidden="true"></span><strong>${enabled?'启用':'停用'}</strong></button></td></tr>`}).join('');
    ...
    body.querySelectorAll('[data-procedure-toggle]').forEach(button=>button.addEventListener('click',event=>{event.stopPropagation();const id=button.dataset.procedureToggle,next=!procedureEnabled.get(id);procedureEnabled.set(id,next);persistProcedureStatus();renderProcedures(list);toast(id+' 已'+(next?'启用':'停用'));}));
  }
