...
(function(){
  ...
  function setFormMode(mode,config){
    ...
    formTitle.textContent=editingMap?'编辑地图信息':'修改实验室名称';nameInput.value=labSummary?.name||'';
  ...
  async function openMapEditor(trigger){
    if(labSummary?.draft){setFormMode('map',labSummary.draft);openModal('spaceFormModal');return}
    if(!labSummary?.published)return showToast('当前没有可编辑的地图配置');
    trigger.disabled=true;trigger.setAttribute('aria-busy','true');
    try{
      await request('/api/lab/drafts',{method:'POST'});
      const result=await request('/api/lab'),lab=result.data||null,draft=lab?.draft;
      if(!draft)throw new Error('创建成功后未获取到草稿配置');
      renderLab(lab);setFormMode('map',draft);openModal('spaceFormModal');showToast('已进入地图编辑');
    }catch(error){console.error('准备地图编辑失败',error);showToast('打开地图编辑失败：'+error.message)}
  ...
  function renderLab(lab){
    ...
    const config=lab.published||lab.draft,isPublished=Boolean(lab.published&&config===lab.published),map=config?.map||{},counts=config?.counts||{},row=document.createElement('tr');twoLineCell(row,lab.name,'唯一实验室');twoLineCell(row,(map.name||'-')+(map.version?' '+map.version:''),map.imageUrl||'-');textCell(row,'机台 '+(counts.machineCount??0)+' · 节点 '+(counts.nodeCount??0)+' · 连线 '+(counts.linkCount??0));
    textCell(row,(counts.pointCount??0)+' 个点位');
    const statusCell=document.createElement('td'),status=document.createElement('span');status.className='badge '+(isPublished?'badge-green':config?'badge-blue':'');status.textContent=isPublished?'已发布 R'+config.revision:config?'草稿 R'+config.revision:'未配置';statusCell.appendChild(status);row.appendChild(statusCell);
    const actionCell=document.createElement('td'),actions=document.createElement('div');actions.className='row-actions';actions.dataset.agvActionMenu='icon';const edit=iconAction('i-edit','编辑地图',trigger=>openMapEditor(trigger));edit.disabled=!config;const view=iconAction('i-map','查看地图',trigger=>showMapPreview(config,trigger));view.disabled=!config;const detail=actionButton('配置详情','',()=>showConfigDetails(config,isPublished?'已发布配置':'当前草稿'));detail.disabled=!config;actions.append(edit,view,detail);if(config)actions.appendChild(actionButton('配置通行/机台','blue',()=>location.href=configUrl('passage-rules.html',config)));if(lab.draft)actions.append(actionButton('校验','',trigger=>validateConfig(lab.draft,trigger)),actionButton('发布','blue',trigger=>publishConfig(lab.draft,trigger)));else if(lab.published)actions.appendChild(actionButton('创建草稿','',trigger=>createDraft(trigger)));
  ...
  function updateSummary(lab){const cards=[...document.querySelectorAll('.summary-card')],config=lab&&(lab.published||lab.draft),map=config?.map||{},counts=config?.counts||{};if(cards[0]){cards[0].querySelector('strong').textContent=lab?((lab.name||'-')+' / '+(map.name||'-')):'-';cards[0].querySelector('p').textContent=lab?'当前实验室空间与其发布地图':'暂无实验室数据'}if(cards[1]){cards[1].querySelector('strong').textContent=config?('1 : '+(counts.pointCount??0)):'-';cards[1].querySelector('p').textContent='一张地图可定义多个导航点'}if(cards[2]){cards[2].querySelector('strong').textContent=(counts.machineCount??0)+' / '+(counts.nodeCount??0)+' / '+(counts.pointCount??0);cards[2].querySelector('p').textContent='机台 / 通行节点 / 导航点'}}
  ...
  window.__labSpaceApi={summaryEndpoint:apiUrl('/api/lab'),uploadEndpoint:apiUrl('/api/files/images'),reload:loadLab,get configId(){return configIdOf(labSummary?.published||labSummary?.draft)}};
