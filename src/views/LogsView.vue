...
const ids = Array.from({ length:7 }, (_,index) => `LOG-20260816-${String(108-index).padStart(4,'0')}`)
...
const rows = computed(() => {
  ...
  return activeRows.value.filter(row => (!level.value || row.level === level.value)
...
function selectTab(key) { active.value = key; level.value = ''; source.value = ''; keyword.value = ''; page.value = 1 }
function reset() { start.value=''; end.value=''; level.value=''; source.value=''; keyword.value=''; page.value=1; toast('查询条件已重置') }
function search() { page.value=1; toast(`当前筛选 ${rows.value.length} 条数据`) }
function openDatePicker(event) { event.currentTarget.previousElementSibling?.showPicker?.() }
...
<template>
  <div class="page-view logs-reference-page">
    ...
    <div class="page-canvas log-page"><section class="log-workspace">
      ...
      <div class="log-body">
        <div class="log-heading-row title-status-bar"><div class="log-title-meta"><h2>系统日志</h2></div><div class="heading-actions"><span class="title-status title-status--danger">{{ rows.length }} 项处理</span><button class="condition-reset" type="button" @click="reset"><img src="/assets/list-icons/refresh.svg" alt="">重置条件</button></div></div>
        <div class="log-filters agv-filter-bar">
          <label class="field agv-filter-field date-field"><span>开始时间</span><div class="date-input-shell"><input v-model="start" type="datetime-local" aria-label="开始时间"><button type="button" aria-label="选择开始时间" @click="openDatePicker"><el-icon><Calendar /></el-icon></button></div></label><label class="field agv-filter-field date-field"><span>结束时间</span><div class="date-input-shell"><input v-model="end" type="datetime-local" aria-label="结束时间"><button type="button" aria-label="选择结束时间" @click="openDatePicker"><el-icon><Calendar /></el-icon></button></div></label><label class="field agv-filter-field"><span>日志级别</span><select v-model="level"><option value="">全部级别</option><option>信息</option><option>警告</option><option>错误</option></select></label><label class="field agv-filter-field"><span>来源模块</span><select v-model="source"><option value="">全部来源</option><option v-for="item in sources" :key="item">{{ item }}</option></select></label><label class="field agv-filter-field"><span>关键词</span><input v-model="keyword" placeholder="日志号 / 事件 / 订单 / 任务" @keydown.enter="search"></label>
        </div>
        <div class="table-wrap log-table"><table aria-label="系统日志列表"><thead><tr><th>发生时间</th><th>等级</th><th>来源模块</th><th>事件</th><th>关联对象</th><th>结果</th><th class="operation-column">操作</th></tr></thead><tbody><tr v-for="row in rows" :key="`${active}-${row.id}`" tabindex="0" :aria-label="`查看日志详情：${row.id}`" data-log-index @dblclick="open(row)" @keydown.enter="open(row)"><td>{{ row.time }}</td><td><span :class="['tag',row.levelClass]">{{ row.level }}</span></td><td>{{ row.source }}</td><td class="event-cell">{{ row.event }}</td><td>{{ row.object }}</td><td><span :class="['tag',row.resultClass]">{{ row.result }}</span></td><td class="operation-column"><button class="log-icon-button" type="button" title="查看详情" @click="open(row)"><img src="/assets/list-icons/file-detail.svg" alt="查看详情"></button></td></tr><tr v-if="!rows.length"><td class="no-log" colspan="7">没有符合当前条件的日志</td></tr></tbody></table></div>
        <div class="log-footer"><span>{{ !level&&!source&&!keyword ? '共计' : '当前筛选' }} {{ rows.length }} 条数据</span><div class="pagination-controls"><button class="page-btn" type="button" :disabled="page<=1" @click="changePage(page-1)">‹</button><button v-for="n in [1,2,3,4,5]" :key="n" :class="['page-btn',{active:page===n}]" type="button" @click="changePage(n)">{{ n }}</button><span>…</span><button :class="['page-btn',{active:page===98}]" type="button" @click="changePage(98)">98</button><button class="page-btn" type="button" @click="nextPage">›</button><select v-model="pageSize" class="page-size"><option>20 条/页</option><option>50 条/页</option></select><span>跳至</span><input v-model="jumpPage" class="jump-input" inputmode="numeric" @keydown.enter="jump"><span>页</span></div></div>
      </div>
    </section></div>

    <div v-if="dialogVisible && current" class="modal-overlay open log-detail-overlay" @click.self="close"><section class="modal-card log-detail-modal" role="dialog" aria-modal="true" :aria-label="`系统日志详情 ${current.id}`"><header class="log-detail-title"><h2>系统日志详情·{{ current.id }}</h2><button class="detail-close-x" type="button" aria-label="关闭" @click="close">×</button></header><div class="log-detail-content"><div class="detail-left"><article class="detail-box"><strong>发生时间</strong><p>{{ current.time }}</p></article><article class="detail-box"><div class="detail-label-row"><strong>日志分类</strong><span :class="['tag',current.levelClass]">{{ current.level }}</span></div><p>{{ tabs.find(item=>item.key===active)?.label }}</p></article><article class="detail-box"><strong>来源模块</strong><p>{{ current.source }}</p></article><article class="detail-box"><strong>关联对象</strong><p>{{ current.object }}</p></article></div><div class="detail-right"><article class="detail-box detail-event"><strong>事件</strong><p>{{ current.event }}</p></article><article class="detail-box detail-result"><strong>处理结果</strong><p><span :class="['tag',current.resultClass]">{{ current.result }}</span></p></article><article class="detail-box detail-description"><strong>详细内容</strong><p>{{ current.event }}，任务保持在安全等待点。</p></article></div></div><div class="readonly-note">系统日志用于故障定位和运行追踪，不能在页面中修改或删除；用户操作及配置变更的责任追溯以“用户操作记录”为准。</div><footer class="log-detail-footer"><button class="modal-close" type="button" @click="close">关闭</button></footer></section></div>
...
.logs-reference-page .log-workspace{min-height:793px}.logs-reference-page .log-tabs{height:77px;padding:0 20px}.logs-reference-page .log-tablist{padding:2px}.logs-reference-page .log-tab{width:89px;height:32px;padding:0 10px;font-size:14px}.logs-reference-page .log-body{padding:20px}.logs-reference-page .log-heading-row{align-items:center;margin-bottom:16px}.heading-actions{display:flex;align-items:center;gap:16px}.condition-reset{height:36px;display:inline-flex;align-items:center;gap:6px;padding:0 16px;border:0;border-radius:8px;color:#fff;background:#1577d2;font-size:14px;font-weight:500;cursor:pointer}.condition-reset img{width:18px;height:18px;filter:brightness(0) invert(1)}.logs-reference-page .title-status{min-height:24px!important;padding:2px 12px!important}.logs-reference-page .log-filters{display:flex;gap:16px;margin-bottom:16px!important}.logs-reference-page .log-filters .field{height:40px;flex:1 1 0!important;padding:0 12px}.logs-reference-page .log-filters .field span,.logs-reference-page .log-filters .field input,.logs-reference-page .log-filters .field select{font-size:14px}.logs-reference-page .log-table{border-color:#f5f6f7;border-radius:10px}.logs-reference-page .log-table table{min-width:1180px;font-size:14px}.logs-reference-page .log-table th{height:48px;padding:0 16px;background:rgba(8,24,41,.02);border-color:#f5f6f7;font-size:12px}.logs-reference-page .log-table td{height:56px;padding:0 16px;border-color:#f5f6f7}.logs-reference-page .log-table th:nth-child(1){width:16.3%}.logs-reference-page .log-table th:nth-child(2){width:10.5%}.logs-reference-page .log-table th:nth-child(3){width:13.2%}.logs-reference-page .log-table th:nth-child(4){width:19.4%}.logs-reference-page .log-table th:nth-child(5){width:19.4%}.logs-reference-page .log-table th:nth-child(6){width:16.5%}.operation-column{width:68px!important;text-align:left}.operation-column{white-space:nowrap}.log-icon-button{width:20px;height:24px;display:inline-grid;place-items:center;padding:0;border:0;background:transparent;cursor:pointer}.log-icon-button+.log-icon-button{margin-left:2px}.log-icon-button img{display:block;width:16px;height:16px}.logs-reference-page .log-footer{height:72px;margin:0;padding:20px 0}.logs-reference-page .pagination-controls .page-size{height:32px}.logs-reference-page .page-btn{width:32px;min-width:32px;height:32px}
.date-field .date-input-shell{min-width:0;display:flex;align-items:center;flex:1;gap:6px}.date-field .date-input-shell input{width:0;min-width:0;min-height:38px;flex:1;padding:0;border:0;outline:0;color:#081829;background:transparent;font:inherit;font-size:14px}.date-field .date-input-shell input::-webkit-calendar-picker-indicator{display:none}.date-field .date-input-shell button{width:24px;height:28px;display:grid;place-items:center;flex:none;padding:0;border:0;color:rgba(8,24,41,.42);background:transparent;cursor:pointer}.date-field .date-input-shell button .el-icon{font-size:16px}
.log-detail-overlay{padding:20px}.log-detail-modal{width:min(800px,calc(100vw - 32px));max-height:calc(100vh - 32px);padding:0;overflow:hidden;border-radius:16px;background:#fff}.log-detail-title{height:60px;display:flex;align-items:center;justify-content:space-between;padding:20px 20px 0}.log-detail-title h2{margin:0;font-size:16px;line-height:24px}.detail-close-x{width:24px;height:24px;padding:0;border:0;color:#8a949d;background:transparent;font-size:30px;font-weight:300;line-height:20px;cursor:pointer}.log-detail-content{display:grid;grid-template-columns:minmax(0,1fr) 372px;gap:16px;padding:0 20px 16px}.detail-left,.detail-right{display:grid;gap:16px}.log-detail-modal .detail-box{min-height:70px;box-sizing:border-box;padding:12px 16px;border:1px solid rgba(8,24,41,.06);border-radius:10px}.log-detail-modal .detail-box strong{margin:0;color:#081829;font-size:14px;font-weight:600;line-height:20px}.log-detail-modal .detail-box p{margin:8px 0 0;color:rgba(8,24,41,.48);font-size:12px;line-height:16px}.detail-label-row{display:flex;align-items:center;justify-content:space-between;gap:10px}.detail-event{min-height:98px!important}.detail-result{min-height:98px!important}.detail-description{min-height:98px!important}.readonly-note{margin:0 20px 18px;padding:16px;border-radius:10px;color:#ff711f;background:rgba(255,113,31,.08);font-size:14px;font-weight:500;line-height:20px}.log-detail-footer{display:flex;justify-content:flex-end;padding:24px 12px 16px;border-top:1px solid rgba(8,24,41,.06)}.log-detail-footer .modal-close{height:36px;padding:0 16px;border-radius:8px;color:#081829;background:rgba(8,24,41,.04);font-size:14px;font-weight:500}
@media (max-width:760px) { .logs-reference-page > .page-head { padding:14px; } }
@media(max-width:1500px){.logs-reference-page .log-filters{flex-wrap:wrap}.logs-reference-page .log-filters .field:nth-child(-n+2){flex-basis:calc(50% - 8px)!important}.logs-reference-page .log-filters .field:nth-child(n+3){flex-basis:calc(33.333% - 11px)!important}}@media(max-width:900px){.logs-reference-page .log-filters .field:nth-child(-n+2){flex-basis:100%!important}.logs-reference-page .log-filters .field:nth-child(n+3){flex-basis:calc(50% - 8px)!important}.logs-reference-page .log-filters .field:last-child{flex-basis:100%!important}.log-detail-content{grid-template-columns:1fr}.log-detail-modal{overflow:auto}.detail-right{grid-template-columns:1fr}.heading-actions{align-items:flex-end;flex-direction:column;gap:8px}}@media(max-width:600px){.logs-reference-page .log-filters .field{flex-basis:100%!important}.logs-reference-page .log-filters .field:nth-child(-n+2),.logs-reference-page .log-filters .field:last-child{height:auto!important;min-height:68px!important;align-items:stretch!important;flex-direction:column!important;gap:4px!important;padding:8px 12px!important}.logs-reference-page .log-filters .field input,.logs-reference-page .log-filters .field select{width:100%!important;min-width:0!important}.logs-reference-page .date-field .date-input-shell{width:100%;min-height:28px;flex:none}.logs-reference-page .date-field .date-input-shell input{width:0!important;min-width:0!important;height:28px!important;min-height:28px!important}.logs-reference-page .log-filters .field:last-child input{height:28px!important;min-height:28px!important}.log-detail-content{padding:0 14px 14px}.log-detail-title{padding:16px 14px 0}.readonly-note{margin:0 14px 14px}}
</style>
