...
function jump() { const value = Number(jumpPage.value); if (Number.isInteger(value) && value >= 1 && value <= pageCount.value) page.value = value; else ElMessage.warning(`请输入 1-${pageCount.value} 之间的页码`) }
...
<template>
  <div class="page-view process-reference-page">
    ...
    <div class="page-canvas"><section class="page-panel"><div class="tabs-row"><nav class="tabs"><router-link class="tab-btn active" to="/workflows/processes">流程列表</router-link><i class="tab-divider"/><router-link class="tab-btn" to="/workflows/templates">流程模板</router-link></nav></div><div class="content">
      <div class="list-head"><div><h2>流程列表</h2><p>{{ loading ? '正在加载后端实时数据…' : `后端实时数据 · 当前第 ${page} 页；点击“编辑”维护流程基本信息` }}</p></div></div>
      <label class="filter"><span>流程名称</span><input v-model="keyword" placeholder="流程模板名称"></label>
      ...
      <div class="pager-row"><div class="total">共计 {{ filtered.length }} 条数据</div><div class="pagination"><button class="page-btn" type="button" :disabled="page<=1" @click="page--">‹</button><button v-for="n in pageCount" :key="n" :class="['page-btn',{active:page===n}]" type="button" @click="page=n">{{ n }}</button><button class="page-btn" type="button" :disabled="page>=pageCount" @click="page++">›</button><select v-model="pageSize" class="page-size"><option :value="10">10 条/页</option><option :value="20">20 条/页</option><option :value="50">50 条/页</option></select><span>跳至</span><input v-model="jumpPage" class="jump" inputmode="numeric" @keydown.enter="jump"><span>页</span></div></div>
    </div></section></div>
