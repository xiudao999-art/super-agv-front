...
<template>
  <div class="page-view charging-reference-page">
    ...
    <main class="page-canvas charging-page"><section class="config-panel">
      ...
      <div v-if="pileMode" class="table-wrap" role="tabpanel"><table aria-label="充电桩配置列表"><thead><tr><th>桩编号</th><th>品牌</th><th>型号</th><th>所属空间（地图）</th><th>通信协议</th><th>额定功率</th><th>关联导航点</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="(row,index) in piles" :key="row.code"><td class="code-cell">{{ row.code }}</td><td>{{ row.brand }}</td><td>{{ row.model }}</td><td>{{ row.space }}</td><td>{{ row.protocol }}</td><td>{{ row.power }}</td><td>{{ row.navPoint }}</td><td><span :class="['status-tag',row.status==='禁用'?'disabled':'enabled']">{{ row.status }}</span></td><td><div class="row-actions"><button class="row-btn" type="button" @click="openEditor(index)">编辑</button><button class="row-btn danger" type="button" @click="remove(index)">删除</button></div></td></tr></tbody></table></div>
      <div v-else class="table-wrap" role="tabpanel"><table aria-label="电池配置列表"><thead><tr><th>配置编号</th><th>电池品牌</th><th>电池型号</th><th>电池类型</th><th>额定电压 / 容量</th><th>低电量阈值</th><th>恢复任务阈值</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="(row,index) in batteries" :key="row.code"><td class="code-cell">{{ row.code }}</td><td>{{ row.brand }}</td><td>{{ row.model }}</td><td>{{ row.type }}</td><td>{{ row.specification }}</td><td>{{ row.lowThreshold }}</td><td>{{ row.resumeThreshold }}</td><td><span :class="['status-tag',row.status==='禁用'?'disabled':'enabled']">{{ row.status }}</span></td><td><div class="row-actions"><button class="row-btn" type="button" @click="openEditor(index)">编辑</button><button class="row-btn danger" type="button" @click="remove(index)">删除</button></div></td></tr></tbody></table></div>
