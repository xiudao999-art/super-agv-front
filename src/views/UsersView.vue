    ...
    <main class="page-canvas user-page">
      ...
      <section class="user-panel">
        ...
        <div class="table-wrap"><table aria-label="用户列表"><thead><tr><th>用户</th><th>账号</th><th>所属部门</th><th>角色</th><th>账号来源</th><th>状态</th><th>最近登录</th><th class="col-actions">操作</th></tr></thead><tbody>
          <tr v-for="user in rows" :key="user.account"><td><div class="user-cell"><span :class="['user-avatar',avatarColors[users.indexOf(user)%avatarColors.length]]">{{ user.name.slice(0,1) }}</span><span><strong>{{ user.name }}</strong><small>{{ user.department }}</small></span></div></td><td>{{ user.account }}</td><td>{{ user.department }}</td><td><span class="role-chip">{{ user.role }}</span></td><td>{{ user.source }}</td><td><span :class="['status-tag',user.status==='启用'?'enabled':'disabled']">{{ user.status }}</span></td><td>{{ user.login }}</td><td class="col-actions"><div class="row-actions"><button class="row-btn" type="button" @click="openUserModal(user)">编辑</button><button class="row-btn" type="button" @click="openReset(user)">重置密码</button><button :class="['row-btn',user.status==='停用'?'enable':'danger']" type="button" @click="toggleUser(user)">{{ user.status==='停用'?'启用':'停用' }}</button></div></td></tr>
