...
const allPermissions=['dashboard','orders','storage','robots','map','stations','peripheral','charging-piles','process','anomaly','logs','capacity','user-management','role-view','role-edit','user-assign']
const initialRoles=[
  ...
  { key:'dispatcher',name:'调度管理员',code:'DISPATCHER',description:'可管理订单、流程及现场调度',avatar:'调',color:'blue',users:4,permissions:['dashboard','orders','storage','robots','map','stations','peripheral','charging-piles','process','anomaly','capacity'],enabled:true },
  { key:'operator',name:'运维工程师',code:'OPERATOR',description:'负责异常处理、设备维护和日志排查',avatar:'运',color:'cyan',users:5,permissions:['dashboard','robots','map','stations','peripheral','charging-piles','anomaly','logs','capacity'],enabled:true },
  { key:'viewer',name:'只读访客',code:'VIEWER',description:'仅可查看运行数据和基础配置',avatar:'访',color:'gray',users:1,permissions:['dashboard','orders','storage','robots','map','stations','peripheral','charging-piles','process','capacity','role-view'],enabled:true },
]
const permissionGroups=[
  {title:'运行调度',description:'现场任务与资源调度',icon:'dashboard',items:[['dashboard','运行总览'],['orders','订单管理'],['storage','库位与载具']]},
  {title:'配置中心',description:'实验室与流程基础配置',icon:'map',items:[['robots','机器人与设备'],['map','地图信息'],['stations','机台与点位'],['peripheral','外设设备'],['charging-piles','充电桩与电池配置'],['process','流程与动作']]},
