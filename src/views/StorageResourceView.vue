...
const carrierStatusMeta = {
  IDLE: ['空闲', 'status-completed'], STORED: ['在库', 'status-completed'], PENDING: ['待处理', 'status-waiting'],
  TRANSPORTING: ['运输中', 'status-transporting'], PROCESSING: ['机台处理中', 'status-processing'], IN_USE: ['使用中', 'status-transporting'],
...
function storageTypeStatus(value) {
  const normalized = String(value ?? '').toUpperCase()
  if (['0', 'DISABLED', 'INACTIVE'].includes(normalized)) return ['停用', 'state-disabled']
  return ['启用', 'state-published']
}
function displayTime(item) { return item.lastCheckTime || item.lastScanTime || item.updateTime || item.createTime || '-' }
function locationState(item) {
  const value = String(item.occupancyStatus ?? item.status ?? '').toUpperCase()
  if (['2', 'CONFLICT', 'ABNORMAL'].includes(value)) return ['冲突', 'state-conflict']
  if (['1', 'OCCUPIED'].includes(value)) return ['占用', 'state-occupied']
  return ['空闲', 'state-empty']
}

function locationRecord(value) {
  return locations.value.find((row) => String(row.id) === String(value)) || {}
}
...
async function openAdd() {
  ...
  if (props.resourceKey === 'locations') {
    try {
      locationTypes.value = await listResource('storageTypes')
    } catch (error) {
      return showToast(`库位类型加载失败：${error.message}`)
    }
  }
  openLayer('form')
}
...
function payload() {
  if (props.resourceKey === 'locations') {
    return {
      ...
      locationType: form.locationType === '' ? null : Number(form.locationType),
...
async function save() {
  ...
  if (props.resourceKey === 'locations') {
    if (data.locationType == null) return showToast('请选择库位类型')
    try {
      locationTypes.value = await listResource('storageTypes')
    } catch (error) {
      return showToast(`库位类型校验失败：${error.message}`)
    }
    if (!locationTypes.value.some((item) => Number(item.id) === data.locationType)) {
      form.locationType = ''
      return showToast('所选库位类型已失效，请重新选择')
    }
    data = payload()
  }
...
const formTitle = computed(() => {
  ...
  if (['storageTypes', 'carrierTypes'].includes(props.resourceKey)) return '编辑类型配置'
...
const spaceMapValue = computed({
  get: () => [form.spaceName, form.mapName].filter(Boolean).join(' / '),
  set: (value) => {
    const [space = '', ...mapParts] = String(value || '').split('/').map((item) => item.trim())
    form.spaceName = space
    form.mapName = mapParts.join(' / ')
  },
})

const formSubmitText = computed(() => {
  if (saving.value) return '保存中…'
  if (props.resourceKey === 'carriers' && editingId.value == null) return '扫码并创建载具'
  if (props.resourceKey === 'storageTypes') return editingId.value == null ? '创建类型草稿' : '保存类型配置'
  if (props.resourceKey === 'carrierTypes') return editingId.value == null ? '创建类型草稿' : '保存类型草稿'
  return '保存'
})
  ...
  <div :class="['storage-resource-page', `${props.resourceKey}-page`]">
    ...
    <div class="page-canvas"><section class="page-panel">
      ...
      <div class="content agv-list-page">
        ...
        <div v-if="props.resourceKey === 'carriers'" class="rule-banner">点击“查看位置”可同时查看载具基础信息、所在库位信息及立库、AGV 或机台内部的具体位置示意。</div>
        ...
        <div v-if="props.resourceKey === 'carriers'" class="carrier-filter agv-filter-bar" data-agv-list-filters>
        ...
        <div class="table-wrap">
          <table v-if="props.resourceKey === 'locations'" aria-label="库位列表"><colgroup><col style="width:9.58%"><col style="width:7.47%"><col style="width:11.68%"><col style="width:9.65%"><col style="width:17.8%"><col style="width:10.67%"><col style="width:13.59%"><col style="width:13.59%"><col style="width:5.97%"></colgroup><thead><tr><th>库位编号 /名称</th><th>类型</th><th>所属空间（地图）</th><th>所属设备 / 对象</th><th>地图坐标 / 到达点</th><th>状态</th><th>当前载具</th><th>最近核对</th><th class="col-actions">操作</th></tr></thead><tbody><tr v-if="loading"><td class="api-loading-cell" colspan="9"><span class="api-loading">正在加载库位…</span></td></tr><tr v-for="item in filteredRows" v-else :key="item.id" :class="{ selected: String(item.id) === String(selectedId) }" @click="selectedId = item.id"><td><div class="cell-stack"><strong>{{ item.locationCode || '-' }}</strong><span>{{ item.locationName || '-' }}</span></div></td><td>{{ typeName(item.locationType) }}</td><td><div class="cell-stack"><strong>{{ item.spaceName || '-' }}</strong><span>{{ item.mapName || '-' }}</span></div></td><td>{{ item.ownerName || '-' }}</td><td><div class="cell-stack"><strong class="main-regular">{{ coordinateText(item) }}</strong><span>导航点：{{ item.navPointCode || '-' }}</span></div></td><td><span :class="['status-tag', locationState(item)[1]]">{{ locationState(item)[0] }}</span></td><td>{{ item.currentCarrierCode || '-' }}</td><td>{{ displayTime(item) }}</td><td class="col-actions"><div class="location-actions"><TableActionButton kind="document" label="查看详情" @click.stop="openDetail(item)"/><TableActionButton kind="map" label="地图位置" @click.stop="openMap(item)"/></div></td></tr><tr v-if="!loading && !filteredRows.length"><td class="location-empty" colspan="9">暂无库位数据</td></tr></tbody></table>

          <table v-if="props.resourceKey === 'carriers'" aria-label="载具记录"><thead><tr><th>载具编码</th><th>条码</th><th>载具类型</th><th>当前库位</th><th>载具状态</th><th>关联订单</th><th>最后扫描/更新</th><th class="col-actions">操作</th></tr></thead><tbody><tr v-if="loading"><td class="api-loading-cell" colspan="8"><span class="api-loading">正在加载载具…</span></td></tr><tr v-for="item in filteredRows" v-else :key="item.id" :class="{ selected: String(item.id) === String(selectedId) }" @click="selectedId = item.id"><td>{{ item.carrierCode || '-' }}</td><td>{{ item.barcode || '-' }}</td><td>{{ carrierTypeName(item.carrierTypeId) }}</td><td>{{ locationName(item.currentLocationId) }}</td><td><span :class="['status-tag', carrierStatus(item.carrierStatus)[1]]">{{ carrierStatus(item.carrierStatus)[0] }}</span></td><td>{{ item.relatedOrderCode || '-' }}</td><td>{{ displayTime(item) }}</td><td class="col-actions"><div class="carrier-actions"><TableActionButton kind="view" label="查看详情" @click.stop="openDetail(item)"/><TableActionButton kind="edit" label="编辑" @click.stop="openEdit(item)"/><TableActionButton kind="delete" label="删除" danger :disabled="String(deletingId) === String(item.id)" @click.stop="remove(item)"/></div></td></tr><tr v-if="!loading && !filteredRows.length"><td class="carrier-empty" colspan="8">暂无载具数据</td></tr></tbody></table>

          <table v-if="props.resourceKey === 'storageTypes'" aria-label="库位类型"><thead><tr><th>类型编码</th><th>名称</th><th>容量</th><th>可兼容载具类型</th><th>状态判定来源</th><th>隔离/互斥规则</th><th>状态</th><th class="col-actions">操作</th></tr></thead><tbody><tr v-if="loading"><td class="api-loading-cell" colspan="8"><span class="api-loading">正在加载库位类型…</span></td></tr><tr v-for="item in filteredRows" v-else :key="item.id" :class="{ selected: String(item.id) === String(selectedId) }" @click="selectedId = item.id"><td>{{ item.typeCode || '-' }}</td><td>{{ item.typeName || '-' }}</td><td>{{ item.capacity ?? '-' }}</td><td>{{ item.compatibleCarrierTypes || '-' }}</td><td>{{ item.statusSource || '-' }}</td><td>{{ item.mutexRule || '-' }}</td><td><span :class="['status-tag', storageTypeStatus(item.status)[1]]">{{ storageTypeStatus(item.status)[0] }}</span></td><td class="col-actions"><div class="location-type-actions"><TableActionButton kind="edit" label="编辑类型配置" @click.stop="openEdit(item)"/><TableActionButton kind="delete" label="删除" danger @click.stop="remove(item)"/></div></td></tr><tr v-if="!loading && !filteredRows.length"><td class="location-type-empty" colspan="8">暂无库位类型数据</td></tr></tbody></table>

          <table v-if="props.resourceKey === 'carrierTypes'" aria-label="载具类型"><thead><tr><th>类型编码</th><th>名称</th><th>外形尺寸</th><th>最大重量</th><th>状态判定来源</th><th>条码规则</th><th>兼容库位类型</th><th>状态</th><th class="col-actions">操作</th></tr></thead><tbody><tr v-if="loading"><td class="api-loading-cell" colspan="9"><span class="api-loading">正在加载载具类型…</span></td></tr><tr v-for="item in filteredRows" v-else :key="item.id" :class="{ selected: String(item.id) === String(selectedId) }" @click="selectedId = item.id"><td>{{ item.typeCode || '-' }}</td><td>{{ item.typeName || '-' }}</td><td>{{ item.dimension || '-' }}</td><td>{{ item.maxWeight == null ? '-' : `${item.maxWeight} kg` }}</td><td>{{ item.statusSource || '-' }}</td><td>{{ item.barcodeRule || '-' }}</td><td>{{ item.compatibleLocationTypes || item.remark || '-' }}</td><td><span :class="['status-tag', carrierTypeStatus(item.status)[1]]">{{ carrierTypeStatus(item.status)[0] }}</span></td><td class="col-actions"><div class="carrier-actions"><TableActionButton kind="edit" label="编辑类型配置" @click.stop="openEdit(item)"/><TableActionButton kind="delete" label="删除" danger @click.stop="remove(item)"/></div></td></tr><tr v-if="!loading && !filteredRows.length"><td class="carrier-empty" colspan="9">暂无载具类型数据</td></tr></tbody></table>
        </div>
      </div>
    </section></div>

    <div v-if="modal === 'form'" id="locationFormModal" class="modal-overlay open" @click.self="closeLayer"><section class="modal-card" role="dialog" aria-modal="true"><div class="location-form-head"><h2>{{ formTitle }}</h2><button class="location-form-x" type="button" aria-label="关闭" @click="closeLayer">×</button></div><p v-if="props.resourceKey === 'locations'" class="location-form-tip">固定库位必须形成“所属空间地图—地图坐标—AGV 到达导航点—取放操作点或业务库位地址”的完整定位链。</p><p v-if="props.resourceKey === 'carriers'" class="location-form-tip">{{ editingId == null ? '每个载具具有唯一编号，并通过条码与载具类型、当前位置建立系统记录。' : '可更新载具基础属性；当前位置和库位占用关系必须通过人工修正向导处理。' }}</p><p v-if="props.resourceKey === 'storageTypes'" class="location-form-tip">{{ editingId == null ? '类型用于统一约束容量、兼容载具、状态来源和互斥规则。' : '已发布类型的兼容规则变更需要检查现有库位、载具和运行中任务。' }}</p><p v-if="props.resourceKey === 'carrierTypes'" class="location-form-tip">{{ editingId == null ? '定义外形、重量、条码规则以及可放入的库位类型。' : '已发布类型的兼容规则变更需要检查现有库位、载具和运行中任务。' }}</p><form @submit.prevent="save"><div class="location-form-body">
      <template v-if="props.resourceKey === 'locations'"><section class="location-form-section"><div class="form-grid"><label class="form-field"><span>库位编号</span><input v-model="form.locationCode" required maxlength="64" :disabled="editingId != null" placeholder="请输入库位编号"></label><label class="form-field"><span>库位名称</span><input v-model="form.locationName" required maxlength="100" placeholder="请输入库位名称"></label><label class="form-field"><span>库位类型</span><select v-model="form.locationType" required><option value="">请选择库位类型</option><option v-for="item in locationTypes" :key="item.id" :value="item.id">{{ item.typeName || item.typeCode }}</option></select></label><label class="form-field"><span>所属空间（地图）</span><input v-model="spaceMapValue" placeholder="例如：A / 总览地图 V3.2"></label><label class="form-field"><span>所属设备 / 对象</span><input v-model="form.ownerName" placeholder="请输入所属设备或对象"></label><label class="form-field"><span>坐标类型</span><input v-model="form.coordinateType" placeholder="例如：固定地图坐标"></label><label class="form-field"><span>地图 X（m）</span><input v-model="form.mapX" type="number" step="0.0001" placeholder="请输入地图 X 坐标"></label><label class="form-field"><span>地图 Y（m）</span><input v-model="form.mapY" type="number" step="0.0001" placeholder="请输入地图 Y 坐标"></label><label class="form-field"><span>地图朝向 θ（°）</span><input v-model="form.mapYaw" type="number" step="0.0001" placeholder="请输入地图朝向"></label><label class="form-field"><span>AGV 到达导航点</span><input v-model="form.navPointCode" placeholder="请输入导航点编码"></label><label class="form-field"><span>取放操作点 / 库位地址</span><input v-model="form.operationPoint" placeholder="请输入操作点或库位地址"></label><label class="form-field"><span>兼容载具类型</span><input v-model="form.compatibleCarrierType" placeholder="请输入兼容载具类型"></label><label class="form-field"><span>状态来源</span><input v-model="form.statusSource" placeholder="例如：扫码＋人工确认"></label></div></section></template>
      <div v-if="props.resourceKey === 'carriers'" class="form-grid"><label class="form-field"><span>载具唯一编号</span><input v-model="form.carrierCode" required maxlength="64" :disabled="editingId != null" placeholder="例如 TRAY-000299"></label><label class="form-field"><span>载具条码</span><input v-model="form.barcode" maxlength="100" placeholder="例如 BC-TRAY-000299"></label><label class="form-field"><span>载具类型</span><select v-model="form.carrierTypeId"><option value="">请选择载具类型</option><option v-for="item in carrierTypes" :key="item.id" :value="item.id">{{ item.typeName || item.typeCode }}</option></select></label><label class="form-field"><span>初始位置</span><select v-model="form.currentLocationId" :disabled="editingId != null"><option value="">未分配库位</option><option v-for="item in locations" :key="item.id" :value="item.id">{{ item.locationName || item.locationCode }}</option></select></label></div>
      <div v-if="props.resourceKey === 'storageTypes'" class="form-grid"><template v-if="editingId == null"><label class="form-field"><span>类型编码</span><input v-model="form.typeCode" required maxlength="64" placeholder="请输入类型编码"></label><label class="form-field"><span>名称</span><input v-model="form.typeName" required maxlength="100" placeholder="请输入类型名称"></label><label class="form-field"><span>容量</span><input v-model="form.capacity" type="number" min="1" step="1"></label></template><label class="form-field"><span>兼容载具类型</span><input v-model="form.compatibleCarrierTypes" placeholder="多个类型用顿号分隔"></label><label class="form-field"><span>状态判定来源</span><input v-model="form.statusSource" placeholder="例如：扫码＋人工确认"></label><label class="form-field"><span>互斥规则</span><input v-model="form.mutexRule" placeholder="例如：任务互斥锁"></label></div>
      <div v-if="props.resourceKey === 'carrierTypes'" class="form-grid"><label class="form-field"><span>类型编码</span><input v-model="form.typeCode" required maxlength="64" :disabled="editingId != null" placeholder="例如：TRAY-000299"></label><label class="form-field"><span>名称</span><input v-model="form.typeName" required maxlength="100" placeholder="例如：项目自定义载具"></label><template v-if="editingId == null"><label class="form-field"><span>外形尺寸</span><input v-model="form.dimension" placeholder="按项目填写"></label><label class="form-field"><span>最大重量</span><input v-model="form.maxWeight" type="number" min="0" step="0.001" placeholder="按项目填写"></label><label class="form-field"><span>条码规则</span><input v-model="form.barcodeRule" placeholder="例如：一物一码"></label><label class="form-field"><span>兼容库位类型</span><input v-model="form.remark" maxlength="500" placeholder="例如：临时暂存位"></label></template><label v-else class="form-field"><span>变更说明</span><input v-model="form.remark" maxlength="500" placeholder="请输入本次配置变更说明"></label></div>
    </div><div class="modal-actions"><button type="button" class="modal-close" @click="closeLayer">取消</button><button type="submit" class="modal-primary" :disabled="saving">{{ formSubmitText }}</button></div></form></section></div>

    <div v-if="modal === 'detail' && selected" class="modal-overlay open" @click.self="closeLayer"><section :class="['modal-card', { 'carrier-location-modal': props.resourceKey === 'carriers' }]" role="dialog" aria-modal="true"><div class="detail-modal-head"><h2>{{ props.resourceKey === 'locations' ? `库位详情 · ${selected.locationCode || ''}` : `载具位置 · ${selected.carrierCode || ''}` }}</h2><button class="location-form-x" type="button" aria-label="关闭" @click="closeLayer">×</button></div><div v-if="props.resourceKey === 'locations'" class="detail-grid"><article v-for="item in [['库位编码',selected.locationCode],['库位名称',selected.locationName],['库位类型',typeName(selected.locationType)],['所属空间',selected.spaceName],['所属地图',selected.mapName],['所属设备或区域',selected.ownerName],['坐标类型',selected.coordinateType],['地图坐标',coordinateText(selected)],['AGV 导航点',selected.navPointCode],['取放操作点',selected.operationPoint],['兼容载具类型',selected.compatibleCarrierType],['状态来源',selected.statusSource],['占用状态',Number(selected.occupancyStatus)===1?'占用':'空闲'],['当前载具',selected.currentCarrierCode],['最后核对时间',selected.lastCheckTime]]" :key="item[0]" class="detail-item"><span>{{ item[0] }}</span><strong>{{ item[1] || '-' }}</strong></article><article class="detail-item wide"><span>备注</span><strong>{{ selected.remark || '-' }}</strong></article></div><div v-else class="carrier-location-body"><div class="carrier-location-columns"><div class="carrier-basic-cards"><article class="carrier-info-card"><strong>载具编号/条码</strong><span>{{ selected.carrierCode || '-' }} / {{ selected.barcode || '-' }}</span></article><article class="carrier-info-card"><strong>载具类型</strong><span>{{ carrierTypeName(selected.carrierTypeId) }}</span></article><article class="carrier-info-card"><div><strong>当前状态</strong><span :class="['status-tag', carrierStatus(selected.carrierStatus)[1]]">{{ carrierStatus(selected.carrierStatus)[0] }}</span></div><small>{{ selected.currentLocationId ? '占用' : '未分配' }}</small></article><article class="carrier-info-card"><strong>关联订单</strong><span>{{ selected.relatedOrderCode || '-' }}</span></article></div><div class="carrier-location-info"><h3>库位基础信息</h3><article v-for="item in [['库位编号 / 名称', `${locationRecord(selected.currentLocationId).locationCode || '-'} · ${locationRecord(selected.currentLocationId).locationName || '-'}`],['所属设备 / 对象',locationRecord(selected.currentLocationId).ownerName],['地图位置',locationRecord(selected.currentLocationId).coordinateType || locationRecord(selected.currentLocationId).mapName],['AGV 到达导航点',locationRecord(selected.currentLocationId).navPointCode || '无需独立导航'],['具体位置',locationName(selected.currentLocationId)]]" :key="item[0]" class="carrier-info-card compact"><strong>{{ item[0] }}</strong><span>{{ item[1] || '-' }}</span></article></div></div><section class="carrier-position-panel"><div class="carrier-position-head"><div><strong>{{ locationRecord(selected.currentLocationId).ownerName || '当前位置' }} · 载具缓存结构</strong><span>载具位置随设备实时地图位置移动</span></div><span class="status-tag status-executing">{{ locationRecord(selected.currentLocationId).locationCode || '当前位置' }}</span></div><div class="carrier-position-structure"><strong>{{ locationRecord(selected.currentLocationId).ownerName || '载具位置结构' }}</strong><div class="carrier-position-slots"><article class="active"><b>{{ locationRecord(selected.currentLocationId).locationName || '当前库位' }}</b><span>{{ selected.carrierCode || '-' }}</span></article><article><b>备用缓存位</b><span>空闲</span></article><article><b>机械臂工作区</b><span>禁止占用</span></article></div></div><p>当前载具位于 {{ locationName(selected.currentLocationId) }}；地图位置继承所属设备实时位姿。</p></section></div><div class="modal-actions"><button class="modal-close" @click="closeLayer">关闭</button><router-link v-if="props.resourceKey === 'locations'" class="modal-primary" to="/dashboard">在运行总览中查看</router-link></div></section></div>
...
/* Figma 35082:5578 — 库位主视图 */
.storage-resource-page .page-canvas{padding:24px}.storage-resource-page .page-panel{border-radius:12px}.storage-resource-page .tabs-row{padding:20px}.storage-resource-page .tabs{border-radius:10px;background:#f5f6f7}.storage-resource-page .tab-btn{height:32px;padding:6px 16px;border-radius:8px;font-size:14px;line-height:20px}.storage-resource-page .tab-btn.active{box-shadow:0 1px 3px rgba(21,27,41,.05)}.storage-resource-page .tab-divider{height:14px;margin:0;background:rgba(8,24,41,.12)}
.locations-page .content{padding:20px}.locations-page .rule-banner{margin:0 0 20px;padding:16px 16px 16px 20px;border-radius:10px;color:#ff711f;background:rgba(255,113,31,.08);font-size:14px;font-weight:500;line-height:20px}.locations-page .list-head{align-items:center;margin-bottom:20px}.locations-page .list-heading h2{margin:0 0 4px;font-size:14px;font-weight:600;line-height:20px}.locations-page .list-heading p{font-size:12px;line-height:16px;color:rgba(8,24,41,.48)}.locations-page .list-tools{gap:16px}.locations-page .legend{gap:16px}.locations-page .legend-chip,.locations-page .status-tag{min-width:24px;padding:4px 12px 4px 8px;gap:4px;border-radius:999px;font-size:12px;font-weight:500;line-height:16px}.locations-page .legend-chip:before,.locations-page .status-tag:before{width:5px;height:5px}.locations-page .state-empty{color:#28bd6b;border-color:rgba(40,189,107,.15);background:rgba(40,189,107,.08)}.locations-page .state-occupied{color:#1577d2;border-color:rgba(21,119,210,.15);background:rgba(21,119,210,.08)}.locations-page .state-conflict{color:#f24e3f;border-color:rgba(242,78,63,.15);background:rgba(242,78,63,.08)}.locations-page .add-btn{height:36px;gap:6px;padding:8px 16px;border-radius:8px;background:#1577d2;font-size:14px;font-weight:500;line-height:20px}.locations-page .add-btn .icon{width:18px;height:18px}.locations-page .table-wrap{border-color:#f5f6f7;border-radius:0}.locations-page table{min-width:1472px;table-layout:fixed;font-size:14px}.locations-page th,.locations-page td{padding:0 16px;border-color:#f5f6f7}.locations-page th{height:48px;background:rgba(8,24,41,.02);font-size:12px;line-height:16px}.locations-page td{height:68px;font-size:14px;line-height:20px}.locations-page .cell-stack strong{font-size:14px;font-weight:500;line-height:20px}.locations-page .cell-stack span{margin-top:4px;font-size:12px;line-height:16px;color:rgba(8,24,41,.48)}.locations-page th.col-actions,.locations-page td.col-actions{width:88px}.locations-page .location-actions{position:relative;gap:4px}.locations-page .location-actions :deep(.table-action-button){width:20px;height:24px;border:0;border-radius:4px}.locations-page .location-actions :deep(.table-action-button svg){width:16px;height:16px}.more-action{position:relative}.more-button{width:20px;height:24px;display:flex;align-items:center;justify-content:center;gap:2px;padding:0;border:0;border-radius:4px;color:#1577d2;background:transparent;cursor:pointer}.more-button:hover{background:rgba(21,119,210,.08)}.more-button i{width:2px;height:2px;border-radius:50%;background:currentColor}.more-menu{position:absolute;top:28px;right:0;z-index:5;min-width:84px;padding:4px;border:1px solid #edf0f2;border-radius:8px;background:#fff;box-shadow:0 8px 24px rgba(8,24,41,.12)}.more-menu button{width:100%;height:30px;padding:0 10px;border:0;border-radius:5px;text-align:left;background:transparent;font-size:12px;cursor:pointer}.more-menu button:hover{background:#f5f6f7}.more-menu button.danger{color:#f24e3f}
@media(max-width:760px){.storage-resource-page .page-canvas{padding:12px}.storage-resource-page .tabs-row{padding:12px}.locations-page .content{padding:16px 12px}.locations-page .rule-banner{padding:14px;font-size:12px}.locations-page .list-head{margin-bottom:14px}}

/* Figma 35080:3541 — 新增/编辑库位 */
.locations-page #locationFormModal{padding:24px}.locations-page #locationFormModal .modal-card{width:min(600px,calc(100vw - 32px));max-height:calc(100vh - 48px);padding:20px 20px 0;border-radius:16px;background:#fff}.locations-page #locationFormModal .location-form-head{display:flex;align-items:flex-start;padding:0;border:0}.locations-page #locationFormModal .location-form-head h2{margin:0;font-size:16px;font-weight:600;line-height:24px}.locations-page #locationFormModal .location-form-x{width:24px;height:24px;display:grid;padding:0;border:0;border-radius:4px;color:#89939d;background:transparent;font-size:25px;font-weight:300;line-height:20px}.locations-page #locationFormModal .location-form-x:hover{background:#f5f6f7}.locations-page #locationFormModal .location-form-tip{margin:16px 0;color:rgba(8,24,41,.84);font-size:14px;line-height:20px}.locations-page #locationFormModal form{min-height:0;display:flex;flex-direction:column}.locations-page #locationFormModal .location-form-body{max-height:calc(100vh - 210px);padding:0 0 16px;background:#fff}.locations-page #locationFormModal .location-form-section{padding:0;border:0;background:#fff}.locations-page #locationFormModal .form-grid,.locations-page #locationFormModal .form-grid.coordinate-grid{grid-template-columns:1fr;gap:16px}.locations-page #locationFormModal .form-field{gap:8px}.locations-page #locationFormModal .form-field>span{color:rgba(8,24,41,.48);font-size:14px;font-weight:400;line-height:20px}.locations-page #locationFormModal .form-field input,.locations-page #locationFormModal .form-field select{height:40px;padding:8px 12px;border-color:rgba(8,24,41,.1);border-radius:8px;font-size:14px;line-height:20px}.locations-page #locationFormModal .modal-actions{margin:0 -20px;padding:24px 12px 16px;border-top:1px solid rgba(8,24,41,.06);background:#fff}.locations-page #locationFormModal .modal-close,.locations-page #locationFormModal .modal-primary{height:36px;padding:8px 16px;border-radius:8px;font-size:14px;font-weight:500;line-height:20px}
@media(max-width:760px){.locations-page #locationFormModal{padding:10px}.locations-page #locationFormModal .modal-card{width:100%;max-height:calc(100vh - 20px);padding:18px 16px 0}.locations-page #locationFormModal .location-form-body{max-height:calc(100vh - 190px)}.locations-page #locationFormModal .modal-actions{margin:0 -16px}}

/* Figma 35086:6774 — 载具列表与弹框 */
.carriers-page .rule-banner{margin:0 0 20px;padding:16px 20px;border-radius:10px;color:#ff711f;background:rgba(255,113,31,.08);font-size:14px;font-weight:500;line-height:20px}.carriers-page td{height:56px}.carriers-page .status-tag{min-width:24px;height:auto;gap:4px;padding:4px 12px 4px 8px;border-color:currentColor;border-radius:999px;font-size:12px;font-weight:500;line-height:16px}.carriers-page .status-tag::before{width:5px;height:5px}.carriers-page .status-completed{color:#28bd6b;border-color:rgba(40,189,107,.15);background:rgba(40,189,107,.08)}.carriers-page .status-waiting{color:#f6b714;border-color:rgba(246,183,20,.15);background:rgba(246,183,20,.08)}.carriers-page .status-transporting{color:#1577d2;border-color:rgba(21,119,210,.15);background:rgba(21,119,210,.08)}.carriers-page .status-processing{color:#009fe3;border-color:rgba(0,159,227,.15);background:rgba(0,159,227,.08)}.carriers-page .status-failed{color:#f24e3f;border-color:rgba(242,78,63,.15);background:rgba(242,78,63,.08)}
.carriers-page #locationFormModal{padding:24px}.carriers-page #locationFormModal .modal-card{width:min(600px,calc(100vw - 32px));max-height:calc(100vh - 48px);padding:20px 20px 0;border-radius:16px;background:#fff}.carriers-page #locationFormModal .location-form-head{display:flex;align-items:flex-start;padding:0;border:0}.carriers-page #locationFormModal .location-form-head h2{margin:0;font-size:16px;font-weight:600;line-height:24px}.carriers-page #locationFormModal .location-form-x{width:24px;height:24px;display:grid;padding:0;border:0;border-radius:4px;color:#89939d;background:transparent;font-size:25px;font-weight:300;line-height:20px}.carriers-page #locationFormModal .location-form-tip{margin:16px 0;color:rgba(8,24,41,.84);font-size:14px;line-height:20px}.carriers-page #locationFormModal form{min-height:0;display:flex;flex-direction:column}.carriers-page #locationFormModal .location-form-body{max-height:calc(100vh - 210px);padding:0 0 20px;background:#fff}.carriers-page #locationFormModal .form-grid{grid-template-columns:1fr;gap:16px}.carriers-page #locationFormModal .form-field{gap:8px}.carriers-page #locationFormModal .form-field>span{color:rgba(8,24,41,.48);font-size:14px;line-height:20px}.carriers-page #locationFormModal .form-field input,.carriers-page #locationFormModal .form-field select{height:36px;padding:8px 12px;border-color:rgba(8,24,41,.1);border-radius:8px;font-size:14px}.carriers-page #locationFormModal .modal-actions{margin:0 -20px;padding:24px 12px 16px;border-top:1px solid rgba(8,24,41,.06);background:#fff}.carriers-page #locationFormModal .modal-close,.carriers-page #locationFormModal .modal-primary{height:36px;padding:8px 16px;border-radius:8px;font-size:14px;font-weight:500}
.detail-modal-head{display:flex;align-items:flex-start;justify-content:space-between}.detail-modal-head .location-form-x{width:24px;height:24px;display:grid;place-items:center;padding:0;border:0;border-radius:4px;color:#89939d;background:transparent;font-size:25px;line-height:20px;cursor:pointer}.carrier-location-modal{width:min(800px,calc(100vw - 32px));max-height:calc(100vh - 48px);padding:20px 20px 0;border-radius:16px;background:#fff}.carrier-location-modal h2{margin:0 0 16px;font-size:16px;line-height:24px}.carrier-location-body{min-height:0;overflow:auto;padding-bottom:20px}.carrier-location-columns{display:grid;grid-template-columns:1fr 1fr;gap:16px}.carrier-basic-cards,.carrier-location-info{display:grid;gap:16px}.carrier-location-info{grid-template-rows:20px repeat(5,70px)}.carrier-location-info h3{margin:0;font-size:14px;line-height:20px}.carrier-info-card{min-height:70px;display:flex;flex-direction:column;gap:8px;justify-content:center;padding:12px 16px;border:1px solid rgba(8,24,41,.06);border-radius:10px;background:#fff}.carrier-basic-cards .carrier-info-card{min-height:100px}.carrier-info-card strong{font-size:14px;font-weight:500;line-height:20px}.carrier-info-card span,.carrier-info-card small{color:rgba(8,24,41,.48);font-size:12px;line-height:16px}.carrier-info-card>div{display:flex;align-items:center;justify-content:space-between;gap:12px}.carrier-info-card .status-tag{color:#1577d2}.carrier-position-panel{margin-top:16px;padding:12px 16px;border:1px solid rgba(8,24,41,.06);border-radius:10px}.carrier-position-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.carrier-position-head>div{display:grid;gap:8px}.carrier-position-head strong,.carrier-position-structure>strong{font-size:14px;font-weight:500;line-height:20px}.carrier-position-head span{color:rgba(8,24,41,.48);font-size:12px;line-height:16px}.carrier-position-structure{margin-top:20px;padding:12px 16px;border:1px solid rgba(8,24,41,.06);border-radius:10px}.carrier-position-slots{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:20px}.carrier-position-slots article{height:70px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;border:1px solid rgba(8,24,41,.06);border-radius:10px}.carrier-position-slots article.active{color:#1577d2;border-color:#1577d2;background:rgba(21,119,210,.08)}.carrier-position-slots b{font-size:14px;font-weight:500}.carrier-position-slots span{color:rgba(8,24,41,.48);font-size:12px}.carrier-position-slots .active span{color:rgba(21,119,210,.75)}.carrier-position-panel>p{margin:20px 0 0;color:rgba(8,24,41,.48);font-size:10px;line-height:12px;text-align:center}.carrier-location-modal>.modal-actions{margin:0 -20px;padding:24px 12px 16px;border-top:1px solid rgba(8,24,41,.06)}
@media(max-width:760px){.carriers-page #locationFormModal{padding:10px}.carriers-page #locationFormModal .modal-card{width:100%;max-height:calc(100vh - 20px);padding:18px 16px 0}.carriers-page #locationFormModal .modal-actions{margin:0 -16px}.carrier-location-modal{width:100%;padding:16px 14px 0}.carrier-location-columns{grid-template-columns:1fr}.carrier-location-info{grid-template-rows:auto}.carrier-basic-cards .carrier-info-card{min-height:70px}.carrier-position-slots{grid-template-columns:1fr}.carrier-location-modal>.modal-actions{margin:0 -14px}}

/* Figma 35152:9202 — 库位类型 */
.storageTypes-page .content{padding:20px}.storageTypes-page .list-head{align-items:center;margin-bottom:16px}.storageTypes-page .list-heading h2{margin:0;font-size:14px;font-weight:600;line-height:20px}.storageTypes-page .list-tools{gap:16px}.storageTypes-page .action-btn{height:36px;gap:6px;padding:8px 16px;border-radius:8px;background:#1577d2;font-size:14px;font-weight:500;line-height:20px}.storageTypes-page .action-btn .icon{width:18px;height:18px;flex:0 0 auto}.storageTypes-page .table-wrap{border-color:#f5f6f7;border-radius:0}.storageTypes-page table{min-width:1240px;table-layout:fixed;font-size:14px}.storageTypes-page th,.storageTypes-page td{padding:0 16px;border-color:#f5f6f7}.storageTypes-page th{height:48px;background:rgba(8,24,41,.02);font-size:12px;line-height:16px}.storageTypes-page td{height:56px;font-size:14px;line-height:20px}.storageTypes-page th.col-actions,.storageTypes-page td.col-actions{width:88px}.storageTypes-page .location-type-actions{gap:4px}.storageTypes-page .location-type-actions :deep(.table-action-button){width:24px;height:28px;border:0}.storageTypes-page .status-tag{min-width:24px;display:inline-flex;align-items:center;justify-content:center;gap:4px;padding:4px 12px 4px 8px;border:1px solid currentColor;border-radius:999px;font-size:12px;font-weight:500;line-height:16px}.storageTypes-page .status-tag:before{content:"";width:5px;height:5px;border-radius:50%;background:currentColor}.storageTypes-page .state-published{color:#28bd6b;border-color:rgba(40,189,107,.15);background:rgba(40,189,107,.08)}
.storageTypes-page #locationFormModal{padding:24px}.storageTypes-page #locationFormModal .modal-card{width:min(600px,calc(100vw - 32px));max-height:calc(100vh - 48px);padding:20px 20px 0;border-radius:16px;background:#fff}.storageTypes-page #locationFormModal .location-form-head{display:flex;align-items:flex-start;padding:0;border:0}.storageTypes-page #locationFormModal .location-form-head h2{margin:0;font-size:16px;font-weight:600;line-height:24px}.storageTypes-page #locationFormModal .location-form-x{width:24px;height:24px;display:grid;padding:0;border:0;border-radius:4px;color:#89939d;background:transparent;font-size:25px;font-weight:300;line-height:20px}.storageTypes-page #locationFormModal .location-form-tip{margin:16px 0;color:rgba(8,24,41,.84);font-size:14px;line-height:20px}.storageTypes-page #locationFormModal form{min-height:0;display:flex;flex-direction:column}.storageTypes-page #locationFormModal .location-form-body{max-height:calc(100vh - 210px);padding:0 0 20px;background:#fff}.storageTypes-page #locationFormModal .form-grid{grid-template-columns:1fr;gap:16px}.storageTypes-page #locationFormModal .form-field{gap:8px}.storageTypes-page #locationFormModal .form-field>span{color:rgba(8,24,41,.48);font-size:14px;line-height:20px}.storageTypes-page #locationFormModal .form-field input,.storageTypes-page #locationFormModal .form-field select{height:36px;padding:8px 12px;border-color:rgba(8,24,41,.1);border-radius:8px;font-size:14px}.storageTypes-page #locationFormModal .modal-actions{margin:0 -20px;padding:24px 12px 16px;border-top:1px solid rgba(8,24,41,.06);background:#fff}.storageTypes-page #locationFormModal .modal-close,.storageTypes-page #locationFormModal .modal-primary{height:36px;padding:8px 16px;border-radius:8px;font-size:14px;font-weight:500}
@media(max-width:760px){.storageTypes-page .content{padding:16px 12px}.storageTypes-page .list-tools{width:100%;display:grid;grid-template-columns:1fr 1fr}.storageTypes-page .action-btn{justify-content:center;padding:8px}.storageTypes-page #locationFormModal{padding:10px}.storageTypes-page #locationFormModal .modal-card{width:100%;max-height:calc(100vh - 20px);padding:18px 16px 0}.storageTypes-page #locationFormModal .modal-actions{margin:0 -16px}}

/* Figma 35152:9296 — 载具类型；编辑入口按产品确认保留在表格行内 */
.carrierTypes-page .content{padding:20px}.carrierTypes-page .list-head{align-items:center;margin-bottom:16px}.carrierTypes-page .list-heading h2{margin:0;font-size:14px;font-weight:600;line-height:20px}.carrierTypes-page .action-btn{height:36px;gap:6px;padding:8px 16px;border-radius:8px;background:#1577d2;font-size:14px;font-weight:500;line-height:20px}.carrierTypes-page .action-btn .icon{width:18px;height:18px;flex:0 0 auto}.carrierTypes-page .table-wrap{border-color:#f5f6f7;border-radius:0}.carrierTypes-page table{min-width:1320px;table-layout:fixed;font-size:14px}.carrierTypes-page th,.carrierTypes-page td{height:auto;padding:0 16px;border-color:#f5f6f7}.carrierTypes-page th{height:48px;background:rgba(8,24,41,.02);font-size:12px;line-height:16px}.carrierTypes-page td{height:56px;font-size:14px;line-height:20px}.carrierTypes-page th.col-actions,.carrierTypes-page td.col-actions{width:88px}.carrierTypes-page .carrier-actions{gap:4px}.carrierTypes-page .carrier-actions :deep(.table-action-button){width:24px;height:28px;border:0}.carrierTypes-page .status-tag{min-width:24px;display:inline-flex;align-items:center;justify-content:center;gap:4px;padding:4px 12px 4px 8px;border:1px solid currentColor;border-radius:999px;font-size:12px;font-weight:500;line-height:16px}.carrierTypes-page .status-tag:before{content:"";width:5px;height:5px;border-radius:50%;background:currentColor}.carrierTypes-page .state-published{color:#28bd6b;border-color:rgba(40,189,107,.15);background:rgba(40,189,107,.08)}.carrierTypes-page .status-waiting{color:#f24e3f;border-color:rgba(255,113,31,.15);background:rgba(255,113,31,.08)}
.carrierTypes-page #locationFormModal{padding:24px}.carrierTypes-page #locationFormModal .modal-card{width:min(600px,calc(100vw - 32px));max-height:calc(100vh - 48px);padding:20px 20px 0;border-radius:16px;background:#fff}.carrierTypes-page #locationFormModal .location-form-head{display:flex;align-items:flex-start;padding:0;border:0}.carrierTypes-page #locationFormModal .location-form-head h2{margin:0;font-size:16px;font-weight:600;line-height:24px}.carrierTypes-page #locationFormModal .location-form-x{width:24px;height:24px;display:grid;padding:0;border:0;border-radius:4px;color:#89939d;background:transparent;font-size:25px;font-weight:300;line-height:20px}.carrierTypes-page #locationFormModal .location-form-tip{margin:16px 0;color:rgba(8,24,41,.84);font-size:14px;line-height:20px}.carrierTypes-page #locationFormModal form{min-height:0;display:flex;flex-direction:column}.carrierTypes-page #locationFormModal .location-form-body{max-height:calc(100vh - 210px);padding:0 0 20px;background:#fff}.carrierTypes-page #locationFormModal .form-grid{grid-template-columns:1fr;gap:16px}.carrierTypes-page #locationFormModal .form-field{gap:8px}.carrierTypes-page #locationFormModal .form-field>span{color:rgba(8,24,41,.48);font-size:14px;line-height:20px}.carrierTypes-page #locationFormModal .form-field input,.carrierTypes-page #locationFormModal .form-field select{height:36px;padding:8px 12px;border-color:rgba(8,24,41,.1);border-radius:8px;font-size:14px}.carrierTypes-page #locationFormModal .modal-actions{margin:0 -20px;padding:24px 12px 16px;border-top:1px solid rgba(8,24,41,.06);background:#fff}.carrierTypes-page #locationFormModal .modal-close,.carrierTypes-page #locationFormModal .modal-primary{height:36px;padding:8px 16px;border-radius:8px;font-size:14px;font-weight:500}
@media(max-width:760px){.carrierTypes-page .content{padding:16px 12px}.carrierTypes-page .action-btn{justify-content:center;padding:8px}.carrierTypes-page #locationFormModal{padding:10px}.carrierTypes-page #locationFormModal .modal-card{width:100%;max-height:calc(100vh - 20px);padding:18px 16px 0}.carrierTypes-page #locationFormModal .modal-actions{margin:0 -16px}}
</style>
