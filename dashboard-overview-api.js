(function(){
  ...
  const endpoint=(apiBaseUrl?apiBaseUrl.replace(/\/$/,''):'')+'/api/home-test/overview';
  ...
  style.textContent='.status-pill.offline{color:#8d949c;border-color:#e1e5e8;background:#f7f8f9}.status-pill.loading{color:#71808d;border-color:#dfe5ea;background:#f7f9fa}.module-card.offline{filter:grayscale(.75);opacity:.72}.module-copy span.offline{color:#8d949c}.overview-loading{color:#87929c!important}.module-loading,.module-error{grid-column:1/-1;min-height:96px;display:flex;align-items:center;justify-content:center;gap:10px;border:1px dashed #dfe5ea;border-radius:9px;color:#7a858f;background:#fafbfc;font-size:12px}.module-loading:before{content:"";width:17px;height:17px;border:2px solid #dbe8f2;border-top-color:var(--blue);border-radius:50%;animation:overview-loading-spin .7s linear infinite}.module-error{color:var(--red);border-color:#f0d5d3;background:#fff8f7}@keyframes overview-loading-spin{to{transform:rotate(360deg)}}';
  ...
  async function load(){
    ...
    setLoading();
    try{
      const response=await fetch(endpoint,{headers:{Accept:'application/json'},signal:activeController.signal});
      const result=await parseApiResponse(response);checkApiResult(response,result);
