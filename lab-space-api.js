(function(){
  ...
  async function request(path,options){const response=await fetch(apiUrl(path),{headers:{Accept:'application/json',...(options?.body?{'Content-Type':'application/json'}:{})},...options});const result=await parseApiResponse(response);checkApiResult(response,result);return result}
  ...
  async function uploadMapImage(file,signal){validateImageFile(file);const body=new FormData();body.append('file',file,file.name);const response=await fetch(apiUrl('/api/files/images'),{method:'POST',headers:{Accept:'application/json'},body,signal}),result=await parseApiResponse(response);checkApiResult(response,result);const imageUrl=result.data?.imageUrl;if(!imageUrl)throw new Error('上传接口未返回 imageUrl');mapImageInput.value=imageUrl;mapUploadStatus.textContent='上传成功：'+imageUrl;return imageUrl}
